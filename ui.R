...
shinyUI(fluidPage(theme = shinytheme("flatly"),
  titlePanel(HTML('<b><font size="6"color="#42616E">NIA - Customer Experience Index 1.0</font></b>')),
  fluidRow(
    column(4, wellPanel(HTML('<p><img width=30% height=30% src="tm.gif"/></p>'),
                                                
      htmlOutput("caption"),
      selectInput('zone', 'Zone :', choices=c("ZONE CYBERJAYA")),
      selectInput('exchange', 'Exchange :', choices=c("All Exchange","CBJ","KLIA","PUJ","SKT")),
      selectInput("user", "User :",choices=c("All Customer","psauauto@unifibiz","esteccyber@unifibiz","skycityhotel@unifibiz
","haniabdu5@unifi")),
    ...
    column(8,
           ...
           wellPanel(HTML('<b>Customer Records</b><br><br>'),div(dataTableOutput("cei_table"), style = "font-size:80%")),
           wellPanel(
           HTML('<b>FDC Location</b><br><br>'),
           HTML('<table width="100%" border="1"><tr><th scope="row">'),plotOutput("maps", width = "100%", height = "600px"),HTML('</th></tr></table>')),
           wellPanel(HTML('<b>Cyberjaya Exchange Performance</b>'),
